...
const addTodoCommand = commandFactory<{ label: string }>(
  ({ state, payload: { label } }) => {
...
const deleteTodoCommand = commandFactory<{ id: string }>(
  ({ state, payload: { id } }) => {
    if (state.todos) {
...
const toggleTodoCommand = commandFactory<{ id: string }>(
  ({ state, payload: { id } }) => {
    if (state.todos) {
      const index = state.todos.findIndex(findTodo(id));
      if (index !== -1) {
...
const todoInputCommand = commandFactory<{ current: string }>(
  ({ state, payload }) => {
...
const todoEditModeCommand = commandFactory<{ id: string; label: string }>(
  ({ state, payload: { id, label } }) => {
    state.editingId = id;
...
const updateTodoCommand = commandFactory<{ label: string }>(
  ({ state, payload: { label } }) => {
    state.editingLabel = label;
  }
);

const addTodo = createProcess("add-todo", [
...
const toggleAllTodos = createProcess("toggle-all-todos", [
  toggleAllTodosCommand,
]);
...
const saveTodo = createProcess("save-todo", [
  saveTodoCommand,
  todoReadModeCommand,
]);
const updateTodoInput = createProcess("update-todo-input", [updateTodoCommand]);
const clearCompleted = createProcess("clear-completed", [
  ...
  return (
    <section classes="todoapp">
      <header>
        ...
        <input
          checked={
            todos && completedCount > 0 && todos.length === completedCount
          }
          type="checkbox"
          classes="toggleAll"
          onchange={() => {
