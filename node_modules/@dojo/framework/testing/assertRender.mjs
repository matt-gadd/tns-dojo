...
function formatObject(obj, depth = 0, style) {
...
function format(nodes, depth = 0) {
    nodes = Array.isArray(nodes) ? nodes : [nodes];
    const nodeString = nodes.reduce((str, node, index) => {
...
export function assertRender(actual, expected) {
    ...
    let diffFound = false;
    const parsedDiff = diffResult.reduce((result, part) => {
        if (part.added) {
            diffFound = true;
