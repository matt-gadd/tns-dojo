(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "tslib", "../shim/global", "../core/has", "../shim/WeakMap", "../shim/Set", "../shim/Map", "../shim/array", "./Registry", "./diff", "./RegistryHandler"], factory);
    }
    ...
    // @ts-ignore
    var scope = typeof __DOJO_SCOPE === 'string' ? __DOJO_SCOPE : 'dojo_scope';
    ...
    function renderer(renderer) {
        ...
        function mount(mountOptions) {
            ...
            if (!domNode) {
                if (has_1.default('dojo-debug') && domNode === null) {
            ...
            if (replace || (currentLength === 0 && !_mountOptions.merge)) {
                ...
                if (newIndex < nextLength) {
                    ...
                    if (currentWrapper && same(currentWrapper, nextWrapper)) {
                        ...
                        has_1.default('dojo-debug') && current.length && distinguishableCheck(next, newIndex);
                        instructions.push({ current: undefined, next: nextWrapper });
                        newIndex++;
                    }
                    else if (findIndexOfChild(next, currentWrapper, newIndex + 1) === -1) {
                        has_1.default('dojo-debug') && distinguishableCheck(current, oldIndex);
                        instructions.push({ current: currentWrapper, next: undefined });
                        oldIndex++;
                    }
                    else {
                        has_1.default('dojo-debug') && distinguishableCheck(next, newIndex);
                        has_1.default('dojo-debug') && distinguishableCheck(current, oldIndex);
                        instructions.push({ current: currentWrapper, next: undefined });
                        instructions.push({ current: undefined, next: nextWrapper });
                        oldIndex++;
                ...
                if (currentLength > oldIndex && newIndex >= nextLength) {
                    for (var i = oldIndex; i < currentLength; i++) {
                        has_1.default('dojo-debug') && distinguishableCheck(current, i);
                        instructions.push({ current: current[i], next: undefined });
                    }
                }
            }
            ...
            if (!next.domNode) {
                if (next.node.domNode) {
                    ...
                    if (isBody) {
                        ...
                        next.domNode = global_1.default.document.head;
                    }
        ...
        function _removeDom(_a) {
            var current = _a.current;
            var isSpecial = isSpecialWrapper(current);
            ...
            if (current.hasAnimations || isSpecial) {
                return {
                    ...
                    dom: isSpecial ? undefined : { type: 'delete', current: current }
                };
            }
            if (children) {
                _deferredRenderCallbacks.push(function () {
