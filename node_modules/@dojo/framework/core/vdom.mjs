...
export function renderer(renderer) {
    ...
    function mount(mountOptions = {}) {
		...
		const document = mountOptions.ownerDocument || global.document;
        if (!domNode) {
        ...
        if (!next.domNode) {
            if (next.node.domNode) {
                ...
                if (isBody) {
                    ...
                    if (next.namespace) {
                        next.domNode = _mountOptions.ownerDocument.createElementNS(next.namespace, next.node.tag);
                    }
                    else {
                        next.domNode = _mountOptions.ownerDocument.createElement(next.node.tag, {}, next.node.properties);
                    }
                }
                else if (next.node.text != null) {
                    next.domNode = _mountOptions.ownerDocument.createTextNode(next.node.text);
                }
            }
